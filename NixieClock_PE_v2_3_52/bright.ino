...
void backlBrightTick() {
  static uint8_t nightColorShown = 255;              // v2.3.51: какой цвет сейчас выведен в режиме SETNIGHT
  // v2.3.51: предупреждение "время не выставлено" (RTC терял питание): красный и зелёный
  // поочерёдно RTC_WARN_TIME_MS, раз в RTC_WARN_PERIOD_MIN минут и при включении.
  // Ночью не показывается. Раньше: 6 с красным только при включении, причём таймер не
  // перезапускался - на деле 4...5 с, а после стартового теста предупреждения не было вовсе.
  if (rtcWarnRequest && curMode == SHTIME && !rtcWarnActive) {
    rtcWarnRequest = false;
    if (!isNightNow) {
      rtcWarnActive = true;
      rtcWarnStart = millis();
      rtcWarnState = true;
      rtcWarnBlinkTimer.reset();
  ...
  if (rtcWarnActive) {
    if (curMode != SHTIME || millis() - rtcWarnStart >= RTC_WARN_TIME_MS) {
      rtcWarnActive = false;
      ...
      if (rtcWarnBlinkTimer.isReady()) {
        rtcWarnState = !rtcWarnState;
        digitalWrite(BACKLB, 0);
        if (rtcWarnState) {
          digitalWrite(BACKLG, 0);
          setPWM(BACKLR, getPWM_CRT(255));
  ...
  switch (curMode) {
    ...
    case SETNIGHT:                                // (8) v2.3.51: настройка дня/ночи: красный - день, синий - ночь, зелёный - выкл
      if (chBL || nightColorCode != nightColorShown) {
        chBL = false;
        nightColorShown = nightColorCode;
        digitalWrite(BACKLR, 0);
        digitalWrite(BACKLG, 0);
        digitalWrite(BACKLB, 0);
        setPWM(nightColorCode == 0 ? BACKLR : (nightColorCode == 1 ? BACKLB : BACKLG), getPWM_CRT(BL_SIGNAL_LEVEL));
      }
      break;
 ...
 */
void changeBright() {
  // v2.3.51: без плавного перехода - яркость меняется скачком. День/ночь определяют два
  // времени (dayStartMin, nightStartMin, см. nightMode.ino). Если оба 00:00 - ночь не наступает.
  // % 1440: при вызове до переноса минут в часы mins может быть 60 (защита от ложной "ночи").
  const uint16_t t = ((uint16_t)hrs * 60 + mins) % 1440;
  isNightNow = isNightMinute(t);
  if (isNightNow) {
    indiMaxBright = INDI_BRIGHT_N;
    dotMaxBright = DOT_BRIGHT_N;
    backlMaxBright[0] = BACKL_BRIGHT_N_R;
    backlMaxBright[1] = BACKL_BRIGHT_N_G;
    backlMaxBright[2] = BACKL_BRIGHT_N_B;
    rainbowMaxBright = BACKL_BRIGHT_RAINBOW_N;
  } else {
  ...
  // v2.3.51: dotSetMode() сбрасывает dotBrightFlag, а changeBright() вызывается каждую минуту
  // сразу после его установки в calculateTime() - точка не пульсировала первую секунду минуты.
  const bool dotFlagKeep = dotBrightFlag;
  if (curMode == SHTEMP) {
    dotSetMode(tempNegative ? DM_THREE : DM_FULL);
  } else {
    dotSetMode(DOT_ALLOWED ? (alm_set ? DOT_IN_ALARM : DOT_IN_TIME) : DM_NULL);
  }
  dotBrightFlag = dotFlagKeep;
