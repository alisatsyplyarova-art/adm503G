...
inline void DCDCTick() {
  ...
  // v2.3.51: окно мягкого старта закрывается защёлкой. Раньше при переполнении millis()
  // (раз в 49.7 суток) оно открывалось снова и на 5 с пропускало регулирование.
  if (!dcdcStartupDone && (now - dcdcStartupStart >= DCDC_STARTUP_TIME_MS)) dcdcStartupDone = true;
  const bool startupActive = !dcdcStartupDone;
